...
<script>
	...
	function drawChart() {
		...
		var data = google.visualization.arrayToDataTable([
			<?php
			$sql = "SELECT _b_district.name, COUNT(_f_device.id) as id_count FROM _b_district JOIN _f_device ON _b_district.id = _f_device.district where " . $entry . "='" . $branch_value . "' GROUP BY _b_district.id limit 20";
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
