...
sw();
w3.getHttpObject("../Controller/viewer/getFaultyList.php", myFunction2);
...
function goHistory(name, id, branch)
{
	...
	else
	{
		...
		{*/
			...
			setTimeout(()=>{
				get2("../Controller/admin/getDetails2.php?imei="+branch);
				branch = 0;
			}, 2000);
		/*}
		else if(branch>1)
...
function get2(url) {
  ...
  xhttp.onload = function() {
	...
	try{
		...
		var faultDetails = "None";

		if(r['battery-fault'] == "1"){
			faultDetails = "Battery fault";
		}

		if(r['luminary-fault'] == "1"){
			if(faultDetails != "None"){
				faultDetails = ", Luminary fault";
			}else{
				faultDetails = "Luminary fault";
			}
		}
		...
		if(r['panel-fault'] == "1"){
			if(faultDetails != "None"){
				faultDetails = ", Paenl fault";
			}else{
				faultDetails = "Paenl fault";
			}
		}

		w3.id("fault_names").innerHTML = faultDetails;
		...
		if (r['battery-fault'] == "1" || r['luminary-fault'] == "1" || r['panel-fault'] == "1") {
