  ...
  <div class="w3-animate-top w3-section w3-border w3-white w3-text-color m8-fancy" style="display:none;" id="addUser">
	<div class="w3-row" id="id02">
		...
		<div class="w3-col s12 m6 l3 inputPadding" w3-repeat="result">
			<label>{{Title}} :</label>
			<input class="w3-input w3-border m8-fancy" name ="{{ID}}" id="{{ID}}" type="{{Type}}">
		</div>
		<div class="w3-col s12 m6 l3" w3-repeat="select">
			<div w3.style="display:{{View}}" class="inputPadding">
				<label>{{Title}} :</label>
				<select class="w3-input w3-border m8-fancy" name ="{{ID}}" id="{{ID}}" title="{{Branch}}"></select>
			</div>
		</div>
		<div class="w3-col s12 m6 l3 inputPadding">
			<label>Image :</label>
			<input class="w3-input w3-border m8-fancy" name="inputFile" id="inputFile" type="file">
		</div>
	</div>
...
function addUser()
{
	...
	var str = mob1.toString();
	var str1 = mob2.toString();
	var moblen = str.length;
	var moblen2 = str1.length;

	if(moblen >10 || moblen2 >10){
	...
	ajax("../Controller/admin/addUser.php",formData);
}
