		...
		<div class="w3-threequarter" style="display:none;" id="id03">
			...
			<div class="w3-container">
				...
				<div class="w3-row">
					...
					<div class="w3-col s6 m4 l4" style="padding:5px;"><button class="w3-button w3-color m8-fancy" style="width:100%;" onclick='replaceDevice(w3.id("log_imei").innerHTML);'><i class="fa fa-lightbulb-o"></i> Replace Light</button></div>
				</div>
			</div>
		</div>
	</div>
...
<div id="editName" class="w3-modal">
	<div class="w3-modal-content w3-animate-top w3-card-4" style="width:400px;height:auto;height:200px;">
		...
		<div class="w3-padding">
			...
			<div class="w3-padding" id="editDeviceButton"></div>
...
<div id="replaceLight" class="w3-modal">
	<div class="w3-modal-content w3-animate-top w3-card-4" style="width:400px;height:auto;height:200px;">
		<header class="w3-container w3-color">
			<span onclick="document.getElementById('replaceLight').style.display='none'" class="w3-button w3-display-topright">&times;</span>
			<h2 id="replaceDeviceH2"></h2>
		</header>
		<div class="w3-padding">
			<label class="w3-text-color">Please provide the IMEI of the new light.</label>
			<input class="w3-input w3-border" id="replaceImeiValue" placeholder="Enter New IMEI here" type="text">
		</div>
		<div id="replaceLightSave">
...
<script>
	...
	function addEntity() {
		...
		// we can't set the blank value, checking here when adding the entity
		if (entity != "" && entity2 != "") {
	...
	function replaceDevice(dev_id){
		w3.id("replaceLight").style.display = "block";
		w3.id("replaceDeviceH2").innerHTML = 'Are you sure to replace light ' + dev_id + ' ?';
		w3.id("replaceLightSave").innerHTML = '<button class="w3-red w3-button w3-section w3-col s12 m12 l12 m8-fancy" onclick="updateLightImei('+dev_id+')">Save</button>';
	}

	function updateLightImei(dev_id){
		console.log(project_id,
			district_id,
			block_id,
			panchayat_id,
			ward_id);
		var newImei = w3.id("replaceImeiValue").value;
		if(newImei.trim().length == 15 && newImei.trim() != dev_id){
			var params = "imei="+dev_id+"&newimei="+newImei;
			sw();
			if (window.XMLHttpRequest){
				xmlhttp = new XMLHttpRequest();
			} else {
			...
			xmlhttp.open("POST", "../Controller/admin/replaceLightImei.php", true);
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4){
					if (xmlhttp.status == 200) {
						if (xmlhttp.responseText == "1") {
							msg("Success, light replaced successfully!");
				...
				w3.id("replaceImeiValue").value = "";
				w3.id("replaceLight").style.display = "none";
			}
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xmlhttp.setRequestHeader("Content-length", params.length);
			xmlhttp.setRequestHeader("Connection", "close");
			xmlhttp.send(params);
		}else if(newImei.trim().length != 15) {
			w3.id("replaceLight").style.display = "none";
			msg("IMEI length should be 15 characters");
		}else if(newImei.trim() == dev_id) {
			w3.id("replaceLight").style.display = "none";
			msg("New Imei cannot be same as old Imei");
		}
	}
