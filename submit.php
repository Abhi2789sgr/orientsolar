...
//$sql = "SELECT id,name FROM _f_device WHERE dev_id='".$id."' AND active='1'";//device disabled
...
if($bypass)
{
  ...
  if ($conn->query($sql) === TRUE){
    $latestQuery = "SELECT device FROM _g_data_latest WHERE device = '".$id."'";
    ...
    if($latestResult->num_rows > 0){
      ...
      $sqlLatest = "INSERT INTO  _g_data_latest (`device`, `v1`, `v2`, `v3`, `v4`, `v5`, `v6`, `v7`, `v8`, `v9`, `v10`, `v11`, `v12`, `v13`, `v14`, `v15`, `v16`, `v17`, `v18`, `v19`, `v20`) VALUES ('".$id."', '".$q[1]."', '".$q[2]."', '".$q[3]."', '".$q[4]."', '".$q[5]."', '".$q[6]."', '".$q[7]."', '".$q[8]."', '".$q[9]."', '".$q[10]."', '".$q[11]."', '".$q[12]."', '".$q[13]."', '".$q[14]."', '".$q[15]."', '".$q[16]."', '".$q[17]."', '".$q[18]."', '".$q[19]."', '".$q[20]."')";
    } 
    ...
    $changeByte = $q[12];
    if($changeByte[7] == '1'){
      if(intval($faultData[7]) == 1){
        $currDateTime = date('Y-m-d H:i:s');
        $lightStatusQuery = "INSERT INTO `light_on_off` (`device_imei`,`on_status`, `on_time`) VALUES ('".$id."', 1, '".$currDateTime."')";
        $conn->query($lightStatusQuery);
      } else {
        $currDateTime = date('Y-m-d H:i:s');
        $getLastLightStatus = "SELECT * FROM `light_on_off` WHERE `device_imei` = '".$id."' AND `on_status` = 1 AND `off_status` IS NULL ORDER BY id DESC LIMIT 1";
        $resLightStatus = $conn->query($getLastLightStatus);
        if($resLightStatus->num_rows > 0){
          $lightStatusRow = $resLightStatus->fetch_assoc();
          $updateLightStatusQuery = "UPDATE `light_on_off` SET `off_status` = 1, `off_time` = '".$currDateTime."' WHERE id = ".$lightStatusRow['id'];
          $conn->query($updateLightStatusQuery);
        }
      }
    }
