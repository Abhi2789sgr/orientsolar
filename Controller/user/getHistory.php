...
if ($result->num_rows > 0)
{
	...
	if($branch != "_f_device"){
		...
		$deviceCountQuery = "SELECT ".$fieldName." as id, COUNT(id) as devicecount FROM _f_device WHERE active = 1 ".$whereCond."  GROUP BY ".$fieldName;
	}
	...
	if($outp == "") $outp = '{"Name": "N/A", "ID": "0", "DeviceCount": "0", "Branch": "'.$branchID.'"}';
		$outp ='{"result":['.$outp.'],"parent_id":'.$parent.'}';
		echo($outp);
}
else
{
