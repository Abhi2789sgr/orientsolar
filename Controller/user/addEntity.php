...
$sql = "SELECT id,branch_value FROM login where uname='{$uid}' and pass='{$pass}' and type='2' and active=1";
...
if ($result->num_rows > 0)
{
	...
	if( isGet($branch_global) && isGet($id_global) && isGet($entity) )
	{
		...
		if($id_global == "") $id_global  = $row["branch_value"];
		
		if($branch_global >6 || $branch_global < 2)
		{
			echo "2";
		}
		else
