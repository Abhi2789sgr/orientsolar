...
foreach ($devData as $dRow) {
	...
	$originalString = $dRow['name'];
	
	if (strpos($originalString, '/') !== false) {
		// Replace '/' with '-'
		$newString = str_replace('/', '-', $originalString);
	} else {
		$newString = $projectFirstTwo .'-'. $districtFirstTwo .'-'. $blockFirstTwo .'-'. $panchFirstTwo .'-'. $wardFirstTwo .'-' .$dRow['name'];

		if (substr_count($originalString, '-') < 4) {
			$sqlDevice = "UPDATE _f_device SET name ='{$newString}' WHERE dev_id = '{$dRow['dev_id']}'";
