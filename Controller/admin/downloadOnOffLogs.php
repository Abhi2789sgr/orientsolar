...
if (isGet("imei")) {
	...
	if ($imei == 0) {
		...
		$sql = "SELECT light_on_off.* , _f_device.name FROM light_on_off LEFT OUTER JOIN _f_device ON light_on_off.device_imei = _f_device.dev_id WHERE light_on_off.device_imei='{$imei}' AND light_on_off.on_time >= '{$_GET['startDate']} 00:00:00' AND light_on_off.off_time <= '{$_GET['endDate']} 23:59:59' ORDER BY light_on_off.device_imei DESC, light_on_off.id";
	}
